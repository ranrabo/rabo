...
function DayExtras({ selectedDate, selectedDay, blocks, onSelectDate }: { selectedDate: Date; selectedDay: number; blocks: BlockWithMember[]; onSelectDate: (date: Date) => void }) {
  ...
  return <div className="grid gap-6 px-5 py-8 sm:w-[90%] sm:px-6 lg:grid-cols-[minmax(0,1fr)_250px] lg:gap-12">
    <article className="py-6 sm:ml-[104px]"><p className="max-w-2xl font-quote text-[13px] font-normal leading-[1.75] text-ink/70 sm:text-[15px]">“{quote.text}”</p><p className="mt-4 font-display text-[10px] font-bold uppercase tracking-[.14em] text-ink/40">— {quote.source}</p></article>
    <aside className="border-l border-ink/15 pl-5 sm:pl-7"><div className="flex items-center justify-between"><p className="font-display text-[10px] font-bold tracking-[.18em] text-ink/50">{calendarMonth.toLocaleString("en-US", { month: "short" }).toUpperCase()} {year}</p><div className="flex items-center gap-1"><button type="button" onClick={() => shiftMonth(-1)} className="px-1 text-sm leading-none text-ink/35 transition hover:text-ink" aria-label="Previous month">‹</button><button type="button" onClick={() => shiftMonth(1)} className="px-1 text-sm leading-none text-ink/35 transition hover:text-ink" aria-label="Next month">›</button></div></div><div className="mt-4 grid grid-cols-7 gap-y-2 text-center font-display text-[10px] text-ink/55">{["M", "T", "W", "T", "F", "S", "S"].map((day, index) => <span key={`${day}-${index}`} className="font-bold">{day}</span>)}{Array.from({ length: firstWeekday }, (_, index) => <span key={`blank-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => { const day = index + 1; const weekday = (firstWeekday + index) % 7; const isSelected = year === selectedDate.getFullYear() && month === selectedDate.getMonth() && day === selectedDate.getDate(); return <button type="button" key={day} onClick={() => onSelectDate(new Date(year, month, day, 12))} className={`relative mx-auto flex h-6 w-6 items-center justify-center transition hover:bg-slate/15 ${isSelected ? "rounded-full bg-slate font-bold text-paper-deep" : ""}`} style={weekday > 4 && !isSelected ? { color: sundayColor } : undefined} aria-label={`Select ${calendarMonth.toLocaleString("en-US", { month: "long" })} ${day}`}>{day}</button>; })}</div></aside>
  </div>;
}
...
function DayHero({ selectedDate, selectedDay, dateAccent, clock, entries, openSessions, isToday, onToday, highlightedPeople, onToggle }: { selectedDate: Date; selectedDay: number; dateAccent: string; clock: string; entries: BlockWithMember[]; openSessions: SessionWithMember[]; isToday: boolean; onToday: () => void; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
  ...
  return <div className="px-5 pb-6 pt-8 sm:px-6 sm:pb-8 sm:pt-10 lg:pl-[128px]"><div className="grid w-full gap-8 sm:w-[68%] lg:grid-cols-[minmax(330px,.85fr)_minmax(0,1.15fr)] lg:gap-10">
    ...
    <div className="w-full self-center"><div className="w-full max-w-full overflow-x-auto"><TodayList entries={entries} openSessions={openSessions} highlightedPeople={highlightedPeople} onToggle={onToggle} /></div></div>
  </div></div>;
}
...
function TodayList({ entries, openSessions, highlightedPeople, onToggle }: { entries: BlockWithMember[]; openSessions: SessionWithMember[]; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
  ...
  return <div className="grid w-max grid-flow-col grid-rows-3 auto-cols-max gap-x-6 gap-y-1">
...
export function PublicBoard({ today, now, people, blocks, openSessions }: { today: string; now: string; people: PublicPerson[]; blocks: BlockWithMember[]; openSessions: SessionWithMember[] }) {
  ...
  return <div className="min-h-screen px-4 pb-20 sm:px-8" style={{ backgroundImage: `linear-gradient(135deg, ${wash(monthAccent, .18)}, ${wash(monthAccent, .08)} 48%, rgba(255,253,249,.72))` }}>
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <DayExtras selectedDate={selectedDate} selectedDay={selectedDay} blocks={blocks} onSelectDate={(date) => setSelectedDateValue(date.toISOString().slice(0, 10))} />
