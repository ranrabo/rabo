...
function DayExtras({ selectedDate, selectedDay, blocks }: { selectedDate: Date; selectedDay: number; blocks: BlockWithMember[] }) {
  ...
  return <div className="grid gap-6 border-b border-ink/20 px-5 py-8 sm:px-10 lg:grid-cols-[minmax(0,1fr)_250px] lg:gap-12">
...
function DayHero({ selectedDate, selectedDay, dateAccent, monthAccent, clock, selectedMonthYear, entries, openSessions, onPrevious, onNext }: { selectedDate: Date; selectedDay: number; dateAccent: string; monthAccent: string; clock: string; selectedMonthYear: string; entries: BlockWithMember[]; openSessions: SessionWithMember[]; onPrevious: () => void; onNext: () => void }) {
  return <div className="grid gap-8 px-5 pb-6 pt-8 sm:px-10 sm:pb-8 sm:pt-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12">
    <div className="flex min-h-[220px] min-w-0 flex-col justify-between border border-[#EAEAEA] bg-[#FFFDF9]"><div className="flex flex-1 items-center justify-center px-4 py-7"><div className="grid grid-cols-[70px_128px_110px] border border-[#EAEAEA] font-display leading-none" style={{ color: dateAccent }}><div className="flex flex-col items-center justify-center border-r border-[#EAEAEA] px-3 py-4"><span className="text-[11px] font-bold tracking-[.12em]">{selectedDate.toLocaleString("en-US", { month: "short" }).toUpperCase()}</span><span className="mt-2 text-xs font-medium tracking-[.12em]">{selectedDate.getFullYear()}</span></div><div className="flex items-center justify-center border-r border-[#EAEAEA] px-4 py-3 text-[7rem] font-extrabold tracking-[-.1em] sm:text-[8rem]">{selectedDate.getDate()}</div><div className="flex flex-col items-center justify-center gap-2 px-4 py-4 text-ink/70"><span className="text-3xl font-extrabold sm:text-4xl">{["月", "火", "水", "木", "金", "土", "日"][selectedDay]}</span><span className="text-sm font-extrabold tracking-[.16em] text-ink/65 sm:text-base">{WEEKDAYS[selectedDay].toUpperCase()}</span></div></div></div><div className="flex items-center justify-between border-t border-[#EAEAEA] px-4 py-3 font-display text-xs font-extrabold tabular-nums sm:px-6"><span className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-coral" />{clock}<span className="text-[10px] font-medium tracking-[.14em] text-ink/50">LAB TIME</span></span><span className="text-[10px] font-medium tracking-[.14em] text-ink/45">{selectedMonthYear}</span></div></div>
    <div className="self-end"><TodayList entries={entries} openSessions={openSessions} /><div className="mt-4 flex justify-end gap-2"><button aria-label="Previous day" onClick={onPrevious} className="flex h-7 w-7 items-center justify-center border border-ink/20 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronLeft size={14} /></button><button aria-label="Next day" onClick={onNext} className="flex h-7 w-7 items-center justify-center border border-ink/20 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronRight size={14} /></button></div></div>
  </div>;
}
...
export function PublicBoard({ today, now, people, blocks, openSessions }: { today: string; now: string; people: Person[]; blocks: BlockWithMember[]; openSessions: SessionWithMember[] }) {
  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <DayHero selectedDate={selectedDate} selectedDay={selectedDay} dateAccent={dateAccent} monthAccent={monthAccent} clock={clock} selectedMonthYear={selectedMonthYear} entries={currentEntries} openSessions={selectedDateValue === today ? openSessions : []} onPrevious={() => shiftSelectedDate(-1)} onNext={() => shiftSelectedDate(1)} />
      <div className="relative px-8 pb-8 sm:px-12">
        ...
        <div className="grid grid-cols-[52px_16px_minmax(0,1fr)] border-t-2 border-ink/20 sm:grid-cols-[82px_22px_minmax(0,1fr)]" style={{ borderTopColor: wash(monthAccent, .55) }}><HourAxis /><DensityBand entries={currentEntries} color={monthAccent} /><div className="min-w-0"><DayTimeline entries={currentEntries} accent={monthAccent} /></div></div>
      </div>
      ...
      <section className="px-5 pb-10 pt-8 sm:px-10 sm:pt-10">
        ...
        <div className="hidden overflow-hidden border-t-2 border-ink/20 lg:block"><div className="grid grid-cols-[52px_repeat(5,minmax(0,1fr))] divide-x divide-ink/15"><div className="border-b border-ink/15 px-2 py-3 font-display text-[10px] font-bold tracking-[.18em] text-ink/55">TIME</div>{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={day} className={`text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><div className={`border-b border-ink/15 px-3 py-3 ${selectedDay === index ? "text-coral" : ""}`}><span className="font-display text-sm font-extrabold tracking-[.12em]">{day.toUpperCase()}</span><span className="ml-2 font-display text-xs text-ink/50">{dateLabel(index)}</span></div></button>)}<div><HourAxis short /></div>{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={`timeline-${day}`} className={`relative min-w-0 px-1.5 text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><DayTimeline entries={byDay[index]} accent={monthAccent} short /></button>)}</div></div>
        <div className="space-y-0 border-t-2 border-ink/20 lg:hidden">{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={day} className={`flex w-full items-center gap-3 border-b border-ink/15 px-1 py-3 text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><span className="w-14 font-display text-xs font-extrabold tracking-[.1em]">{day.toUpperCase()}<small className="mt-0.5 block font-medium tracking-normal text-ink/55">{dateLabel(index)}</small></span><span className="relative h-12 flex-1 border-b border-ink/25"><span className="absolute inset-y-0 left-0 right-0 flex items-end gap-px">{Array.from({ length: 48 }, (_, slot) => { const start = 7 * 60 + slot * 15; const count = byDay[index].filter(({ block }) => toMinutes(block.startTime) <= start && toMinutes(block.endTime) > start).length; return <i key={slot} className="flex-1 bg-slate" style={{ height: `${count ? Math.max(15, count * 24) : 0}%`, opacity: count ? .78 : 0 }} />; })}</span></span></button>)}</div>
      </section>
