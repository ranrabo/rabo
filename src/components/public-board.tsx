  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <section className="px-5 pb-10 pt-8 sm:px-10 sm:pt-10">
        ...
        <div className="hidden overflow-hidden lg:block"><div className="grid grid-cols-[52px_repeat(5,minmax(0,1fr))] divide-x divide-ink/15"><div className="px-2 py-3 font-display text-[10px] font-bold tracking-[.18em] text-ink/55">TIME</div>{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={day} className={`text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><div className={`px-3 py-3 ${selectedDay === index ? "text-coral" : ""}`}><span className="font-display text-sm font-extrabold tracking-[.12em]">{day.toUpperCase()}</span><span className="ml-2 font-display text-xs text-ink/50">{dateLabel(index)}</span></div></button>)}<div><HourAxis short /></div>{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={`timeline-${day}`} className={`relative min-w-0 px-1.5 text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><DayTimeline entries={byDay[index]} accent={monthAccent} short /></button>)}</div></div>
        <div className="space-y-0 lg:hidden">{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDateValue(dateValueForDay(index))} key={day} className={`flex w-full items-center gap-3 px-1 py-3 text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><span className="w-14 font-display text-xs font-extrabold tracking-[.1em]">{day.toUpperCase()}<small className="mt-0.5 block font-medium tracking-normal text-ink/55">{dateLabel(index)}</small></span><span className="relative h-12 flex-1"><span className="absolute inset-y-0 left-0 right-0 flex items-end gap-px">{Array.from({ length: 48 }, (_, slot) => { const start = 7 * 60 + slot * 15; const count = byDay[index].filter(({ block }) => toMinutes(block.startTime) <= start && toMinutes(block.endTime) > start).length; return <i key={slot} className="flex-1 bg-slate" style={{ height: `${count ? Math.max(15, count * 24) : 0}%`, opacity: count ? .78 : 0 }} />; })}</span></span></button>)}</div>
      </section>
