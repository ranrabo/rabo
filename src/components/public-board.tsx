...
function BlockBar({ entry, lane, laneCount, compact = false, highlighted, onToggle }: { entry: BlockWithMember; lane: number; laneCount: number; compact?: boolean; highlighted: boolean; onToggle: (personId: number) => void }) {
  ...
  return <div className="absolute z-[1] overflow-hidden rounded-[2px] transition hover:z-10" style={{ top: `${dayPosition(block.startTime)}%`, height: `${Math.max(((toMinutes(block.endTime) - toMinutes(block.startTime)) / daySpan) * 100, compact ? 5 : 3.8)}%`, left, width, backgroundColor: wash(color, highlighted ? .32 : .16), boxShadow: highlighted ? `0 0 0 1px ${barColor}` : undefined }}><button type="button" aria-label={`Highlight ${member.fullName}`} onClick={() => onToggle(member.id)} className="absolute inset-y-0 left-0 z-10 w-[3px] transition" style={{ backgroundColor: barColor }} /><Link href={`/people/${member.id}`} title={`${member.fullName}, ${formatTime(block.startTime)}–${formatTime(block.endTime)}`} className="absolute inset-y-0 left-[3px] right-0 overflow-hidden px-2 py-1.5 transition hover:brightness-95"><span className="block truncate font-display text-[11px] font-bold leading-tight">{member.fullName}</span><span className="mt-1 block truncate text-[10px] font-medium text-ink/60">{formatTime(block.startTime)}–{formatTime(block.endTime)}</span></Link></div>;
}

function DayTimeline({ entries, mobile = false, short = false, accent, highlightedPeople, onToggle }: { entries: BlockWithMember[]; mobile?: boolean; short?: boolean; accent: string; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
...
function DayHero({ selectedDate, selectedDay, dateAccent, clock, selectedMonthYear, entries, openSessions, isToday, onToday, highlightedPeople, onToggle }: { selectedDate: Date; selectedDay: number; dateAccent: string; clock: string; selectedMonthYear: string; entries: BlockWithMember[]; openSessions: SessionWithMember[]; isToday: boolean; onToday: () => void; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
  ...
  return <div className="grid gap-8 px-5 pb-6 pt-8 sm:px-10 sm:pb-8 sm:pt-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:gap-12">
    <div className="relative flex min-h-[220px] min-w-0 flex-col justify-between border border-[#EAEAEA] bg-[#FFFDF9]"><div className="flex flex-1 items-center justify-center px-4 py-7"><div className="grid w-full max-w-[420px] grid-cols-[.9fr_1.6fr_1fr] border border-[#EAEAEA] font-display leading-none" style={{ color: dateAccent }}><div className="flex flex-col items-center justify-center border-r border-[#EAEAEA] px-2 py-4 sm:px-3"><span className="whitespace-nowrap text-[11px] font-bold tracking-[.12em]">{selectedDate.toLocaleString("en-US", { month: "short" }).toUpperCase()}</span><span className="mt-2 whitespace-nowrap text-xs font-medium tracking-[.12em]">{selectedDate.getFullYear()}</span></div><div className="flex min-w-0 items-center justify-center border-r border-[#EAEAEA] px-2 py-3 text-[5.75rem] font-extrabold tracking-[-.1em] sm:px-4 sm:text-[7rem]">{selectedDate.getDate()}</div><div className="flex flex-col items-center justify-center gap-2 px-2 py-4 text-ink/70 sm:px-4"><span className="whitespace-nowrap text-3xl font-extrabold sm:text-4xl">{["月", "火", "水", "木", "金", "土", "日"][selectedDay]}</span><span className="whitespace-nowrap text-sm font-extrabold tracking-[.16em] text-ink/65 sm:text-base">{WEEKDAYS[selectedDay].toUpperCase()}</span></div></div></div><div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 font-display text-xs font-extrabold tabular-nums sm:px-6"><span className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-coral" />{clock}</span><span className={`flex items-center gap-2 whitespace-nowrap text-[10px] font-bold tracking-[.08em] text-ink/55 ${isNight ? "moon-glow" : ""}`} title={`${moon.name}${isNight ? " · nighttime" : ""}`}><span className="text-base leading-none" aria-hidden="true">{moon.symbol}</span>{moon.name}</span><span className="whitespace-nowrap text-[10px] font-medium tracking-[.14em] text-ink/45">{selectedMonthYear}</span></div></div>
    <div className="w-max max-w-full self-center justify-self-start px-2 sm:px-5"><div className="mb-4 flex justify-end"><button type="button" onClick={onToday} disabled={isToday} className={`whitespace-nowrap border px-3 py-2 font-display text-[11px] font-bold tracking-[.08em] transition ${isToday ? "border-ink/10 text-ink/25" : "border-ink/20 text-ink/55 hover:border-ink/55 hover:text-ink"}`} aria-label="Return to today" aria-pressed={isToday}>Today</button></div><TodayList entries={entries} openSessions={openSessions} highlightedPeople={highlightedPeople} onToggle={onToggle} /></div>
  </div>;
}
...
function TodayList({ entries, openSessions, highlightedPeople, onToggle }: { entries: BlockWithMember[]; openSessions: SessionWithMember[]; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
  ...
  return <div>
    {rows.length ? rows.map(({ member, start, end }) => <div key={member.id} className="flex w-max max-w-full items-center gap-3 py-1.5"><button type="button" aria-label={`Highlight ${member.fullName}`} onClick={() => onToggle(member.id)} className="h-8 w-1 shrink-0 rounded-[1px] transition" style={{ backgroundColor: highlightedPeople.has(member.id) ? brighten(member.color) : member.color, boxShadow: highlightedPeople.has(member.id) ? `0 0 0 1px ${brighten(member.color)}` : undefined }} /><Link href={`/people/${member.id}`} className="flex min-w-0 items-center gap-3 transition hover:bg-ink/[.03]"><span className="truncate font-display text-sm font-bold">{member.fullName}</span><span className="shrink-0 whitespace-nowrap font-display text-[11px] tabular-nums text-ink/55">{formatTime(start)}–{end === "Now" ? "now" : formatTime(end)}</span></Link></div>) : <p className="border-l-2 border-coral pl-3 text-sm leading-6 text-ink/70">Nobody has hours today.</p>}
  </div>;
}
...
function PeoplePalette({ people, highlightedPeople, onToggle }: { people: PublicPerson[]; highlightedPeople: Set<number>; onToggle: (personId: number) => void }) {
  return <section className="px-5 pb-10 pt-2 sm:px-10"><p className="font-display text-[10px] font-bold tracking-[.18em] text-ink/45">PEOPLE</p><div className="mt-4 flex flex-wrap gap-x-7 gap-y-3">{people.map((member) => <div key={member.id} className="flex items-center gap-2.5"><button type="button" aria-label={`Highlight ${member.fullName}`} onClick={() => onToggle(member.id)} className="h-7 w-1 rounded-[1px] transition" style={{ backgroundColor: highlightedPeople.has(member.id) ? brighten(member.color) : member.color, boxShadow: highlightedPeople.has(member.id) ? `0 0 0 1px ${brighten(member.color)}` : undefined }} /><Link href={`/people/${member.id}`} className="font-display text-sm font-bold transition hover:text-ink/60">{member.fullName}</Link></div>)}</div></section>;
}
...
export function PublicBoard({ today, now, people, blocks, openSessions }: { today: string; now: string; people: PublicPerson[]; blocks: BlockWithMember[]; openSessions: SessionWithMember[] }) {
  ...
  return <div className="min-h-screen px-4 pb-20 sm:px-8" style={{ backgroundColor: wash(monthAccent, .12) }}>
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <DayHero selectedDate={selectedDate} selectedDay={selectedDay} dateAccent={dateAccent} clock={clock} selectedMonthYear={selectedMonthYear} entries={currentEntries} openSessions={selectedDateValue === today ? openSessions : []} isToday={selectedDateValue === today} onToday={() => setSelectedDateValue(today)} highlightedPeople={highlightedPeople} onToggle={togglePerson} />
      <div className="relative px-5 pb-8 sm:px-6">
        ...
        <span className="absolute right-0 top-[42%] z-10 flex h-11 w-11 items-center justify-center font-display text-xl font-extrabold" style={{ backgroundColor: monthAccent, color: "#FFFDF9" }} aria-label={`Month ${selectedMonth}`}>{selectedMonth}</span>
        <div className="mx-auto grid w-full grid-cols-[52px_16px_minmax(0,1fr)] sm:grid-cols-[82px_22px_minmax(0,1fr)]"><HourAxis /><DensityBand entries={currentEntries} color={monthAccent} /><div className="min-w-0"><DayTimeline entries={currentEntries} accent={monthAccent} highlightedPeople={highlightedPeople} onToggle={togglePerson} /></div></div>
      </div>
