  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <div className="grid gap-8 px-5 pb-6 pt-8 sm:px-10 sm:pb-8 sm:pt-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12">
        <div className="flex flex-wrap items-stretch gap-8"><div className="flex min-h-[220px] min-w-0 flex-1 flex-col border border-[#EAEAEA] bg-[#FFFDF9]"><div className="flex flex-1 items-center justify-center px-5 py-7"><div className="flex items-end justify-center font-display leading-none" style={{ color: dateAccent }}><span className="self-end border border-r-0 border-[#EAEAEA] px-3 pb-2 text-2xl font-extrabold sm:text-3xl">{selectedMonth}</span><span className="border border-[#EAEAEA] px-3 text-[6.5rem] font-extrabold tracking-[-.1em] sm:text-[8rem]">{selectedDayNumber}</span><span className="flex flex-col items-center justify-center gap-1 border border-l-0 border-[#EAEAEA] px-4 py-3 text-ink/70"><span className="font-display text-3xl font-extrabold leading-none sm:text-4xl">{selectedKanji}</span><span className="font-display text-sm font-extrabold tracking-[.16em] text-ink/65 sm:text-base">{WEEKDAYS[selectedDay].toUpperCase()}</span></span></div></div><div className="flex items-center justify-center gap-2 border-t border-[#EAEAEA] py-4 font-display text-sm font-extrabold tabular-nums"><span className="h-2 w-2 animate-pulse rounded-full bg-coral" />{clock}<span className="text-[10px] font-medium tracking-[.14em] text-ink/55">LAB TIME · {selectedMonthYear}</span></div></div></div>
        <div className="self-end"><TodayList entries={currentEntries} openSessions={selectedDateValue === today ? openSessions : []} /></div>
      </div>
        <div className="grid gap-8 px-5 pb-8 sm:px-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-10">
        <div className="grid grid-cols-[52px_16px_minmax(0,1fr)] border-t-2 border-ink/20 sm:grid-cols-[82px_22px_minmax(0,1fr)]" style={{ borderTopColor: wash(monthAccent, .55) }}><HourAxis /><DensityBand entries={currentEntries} color={monthAccent} /><div className="relative min-w-0"><div className="pointer-events-none absolute inset-x-0 top-2 z-10 flex justify-between px-2"><button aria-label="Previous day" onClick={() => shiftSelectedDate(-1)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronLeft size={14} /></button><button aria-label="Next day" onClick={() => shiftSelectedDate(1)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronRight size={14} /></button></div><DayTimeline entries={currentEntries} accent={monthAccent} /></div></div>
      </div>
