  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <div className="grid gap-8 border-b-2 border-ink/20 px-5 pb-8 sm:px-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-10">
        <div className="grid grid-cols-[52px_16px_minmax(0,1fr)] border-t-2 border-ink/20 sm:grid-cols-[82px_22px_minmax(0,1fr)]"><HourAxis /><DensityBand entries={currentEntries} /><div className="relative min-w-0"><div className="pointer-events-none absolute inset-x-0 top-2 z-10 flex justify-between px-2"><button aria-label="Previous day" onClick={() => setSelectedDay((selectedDay + 6) % 7)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronLeft size={14} /></button><button aria-label="Next day" onClick={() => setSelectedDay((selectedDay + 1) % 7)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronRight size={14} /></button></div><DayTimeline entries={currentEntries} /></div></div>
      </div>
