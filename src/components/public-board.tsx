  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-[#FFFDF9] shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <div className="grid gap-8 px-5 pb-6 pt-8 sm:px-10 sm:pb-8 sm:pt-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12">
        <div className="flex flex-wrap items-end justify-between gap-6"><div><div className="flex items-end gap-3 sm:gap-5"><div className="flex items-end font-display leading-none" style={{ color: dateAccent }}><span className="self-end border border-[#EAEAEA] border-r-0 px-2 pb-1 text-2xl font-extrabold sm:text-3xl">{selectedMonth}</span><span className="border border-[#EAEAEA] px-2 text-[6.5rem] font-extrabold tracking-[-.1em] sm:text-[8rem]">{selectedDayNumber}</span></div><div className="flex flex-col items-center gap-1 border border-l-0 border-[#EAEAEA] pb-2 pl-3 pr-3"><span className="font-display text-3xl font-extrabold leading-none text-ink/70 sm:text-4xl">{selectedKanji}</span><span className="font-display text-sm font-extrabold tracking-[.16em] text-ink/65 sm:text-base">{WEEKDAYS[selectedDay].toUpperCase()}</span></div></div><p className="mt-3 border-t border-[#EAEAEA] pt-2 font-display text-sm font-medium tracking-[.18em] text-ink/55">{selectedMonthYear}</p><div className="mt-5 flex items-center gap-2 font-display text-sm font-extrabold tabular-nums"><span className="h-2 w-2 animate-pulse rounded-full bg-coral" />{clock}<span className="text-[10px] font-medium tracking-[.14em] text-ink/55">LAB TIME</span></div></div><div className="mt-5 grid max-w-xl grid-cols-2 gap-x-5 sm:grid-cols-3"><p className="col-span-full mb-1 font-display text-[10px] font-bold tracking-[.16em] text-ink/45">PEOPLE</p>{people.map((member) => <Link href={`/people/${member.id}`} key={member.id} className="flex min-w-0 items-center gap-2 border-b border-ink/10 py-1.5 transition hover:text-coral"><span className="h-5 w-1 rounded-[1px]" style={{ backgroundColor: hues[(member.id - 1) % hues.length] }} /><span className="truncate font-display text-xs font-bold">{member.fullName}</span></Link>)}</div></div>
        ...
        <div className="grid grid-cols-[52px_16px_minmax(0,1fr)] border-t-2 border-ink/20 sm:grid-cols-[82px_22px_minmax(0,1fr)]" style={{ borderTopColor: wash(monthAccent, .55) }}><HourAxis /><DensityBand entries={currentEntries} color={monthAccent} /><div className="relative min-w-0"><div className="pointer-events-none absolute inset-x-0 top-2 z-10 flex justify-between px-2"><button aria-label="Previous day" onClick={() => setSelectedDay((selectedDay + 6) % 7)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronLeft size={14} /></button><button aria-label="Next day" onClick={() => setSelectedDay((selectedDay + 1) % 7)} className="pointer-events-auto flex h-7 w-7 items-center justify-center border border-ink/20 bg-[#FFFDF9]/80 text-ink/35 transition hover:border-ink/50 hover:text-ink"><ChevronRight size={14} /></button></div><DayTimeline entries={currentEntries} accent={monthAccent} /></div></div>
      </div>
