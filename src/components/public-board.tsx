  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-paper-deep shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <div className="grid gap-8 px-5 pb-6 pt-8 sm:px-10 sm:pb-8 sm:pt-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12">
        <div className="flex flex-wrap items-end justify-between gap-6"><div><div className="flex items-end gap-3 sm:gap-5"><div className="flex items-end gap-2 font-display leading-none"><span className="self-start pt-1 text-2xl font-extrabold text-slate sm:text-3xl">{selectedMonth}</span><span className="text-[6.5rem] font-extrabold tracking-[-.1em] text-slate sm:text-[8rem]">{selectedDayNumber}</span></div><div className="flex flex-col items-center gap-1 pb-2"><span className="font-display text-3xl font-extrabold leading-none text-ink/70 sm:text-4xl">{selectedKanji}</span><span className="font-display text-sm font-extrabold tracking-[.16em] text-ink/65 sm:text-base">{WEEKDAYS[selectedDay].toUpperCase()}</span></div></div><p className="mt-3 font-display text-sm font-medium tracking-[.18em] text-ink/55">{selectedMonthYear}</p><div className="mt-5 flex items-center gap-2 font-display text-sm font-extrabold tabular-nums"><span className="h-2 w-2 animate-pulse rounded-full bg-coral" />{clock}<span className="text-[10px] font-medium tracking-[.14em] text-ink/55">LAB TIME</span></div><div className="mt-4 flex gap-2"><button onClick={() => setSelectedDay((selectedDay + 6) % 7)} className="flex min-h-11 items-center gap-2 border border-ink/35 px-3 font-display text-xs font-bold tracking-[.04em] transition hover:bg-ink/5"><ChevronLeft size={15} />{WEEKDAYS[(selectedDay + 6) % 7]}</button><button onClick={() => setSelectedDay((selectedDay + 1) % 7)} className="flex min-h-11 items-center gap-2 border border-ink/35 px-3 font-display text-xs font-bold tracking-[.04em] transition hover:bg-ink/5">{WEEKDAYS[(selectedDay + 1) % 7]}<ChevronRight size={15} /></button></div></div></div>
        <div className="self-end"><TodayList entries={currentEntries} openSessions={openSessions} now={clock} todayIndex={selectedDay} actualTodayIndex={todayWeekday} /></div>
      </div>
