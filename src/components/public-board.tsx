...
function DayTimeline({ entries, now, showNow = true, mobile = false, short = false }: { entries: BlockWithMember[]; now: string; showNow?: boolean; mobile?: boolean; short?: boolean }) {
  ...
  return <div className={short ? "relative h-[360px]" : mobile ? "relative h-[408px]" : "relative h-[672px]"}>
    ...
    {Array.from({ length: 13 }, (_, i) => <span key={`label-${i}`} className="pointer-events-none absolute -left-14 -translate-y-1/2 font-display text-[10px] font-medium tabular-nums text-ink/45" style={{ top: `${(i / 12) * 100}%` }}>{timeLabel(7 + i)}</span>)}
  ...
  return <div className="mx-auto max-w-[1400px] px-4 pb-20 sm:px-8">
    <section className="mx-auto max-w-[1280px] overflow-hidden border-x border-b border-ink/20 bg-paper-deep shadow-[0_16px_50px_rgba(43,41,38,.08)]">
      ...
      <div className="grid gap-8 border-b-2 border-ink/20 px-5 pb-8 sm:px-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-10">
        <div className="ml-14 border-t-2 border-ink/20 pt-0 sm:ml-20"><DayTimeline entries={currentEntries} now={clock} /></div>
      ...
      <section className="px-5 pb-10 pt-8 sm:px-10 sm:pt-10">
        ...
        <div className="hidden overflow-hidden border-t-2 border-ink/20 lg:block"><div className="grid grid-cols-5 divide-x divide-ink/15">{WEEKDAYS.slice(0, 5).map((day, index) => <button onClick={() => setSelectedDay(index)} key={day} className={`text-left ${selectedDay === index ? "bg-coral/[.06]" : ""}`}><div className={`border-b border-ink/15 px-3 py-3 ${selectedDay === index ? "text-coral" : ""}`}><span className="font-display text-sm font-extrabold tracking-[.12em]">{day.toUpperCase()}</span><span className="ml-2 font-display text-xs text-ink/50">{dateLabel(index)}</span></div><div className="relative h-[360px] px-1.5 pt-0"><DayTimeline entries={byDay[index]} now={clock} showNow={selectedDay === index} short /></div></button>)}</div></div>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-ink/20 bg-paper-deep px-5 py-4 font-display text-[10px] font-medium tracking-[.14em] text-ink/50 sm:px-10"><span>RABO.YANGRAN.ORG</span><span>07:00–19:00 · AMERICA/NEW_YORK · {totalHours.toFixed(1)} H SCHEDULED</span></footer>
    </section>
  </div>;
}
