...
const palette = ["bg-aqua", "bg-coral", "bg-periwinkle", "bg-[#f6c967]"];
    ...
    <div className="mt-7">
          ...
          {WEEKDAYS.map((day, index) => (
            <div key={day} className="min-w-0">
              ...
              <div className="grid-paper relative h-[360px] p-2">
                ...
                {byDay[index].map(({ block, member }, blockIndex) => {
                  ...
                  return <Link href={`/people/${member.id}`} key={block.id} className={cn("absolute left-2 right-2 overflow-hidden rounded-lg px-2 py-2 text-[11px] font-bold leading-tight text-ink transition hover:z-10 hover:scale-[1.02]", palette[blockIndex % palette.length])} style={{ top: `${top}%`, height: `${height}%` }}><span className="block truncate">{member.fullName}</span><span className="mt-1 block truncate text-[10px] font-semibold opacity-65">{formatTime(block.startTime)}–{formatTime(block.endTime)}</span></Link>;
                })}
              </div>
            </div>
          ))}
      ...
      <div className="space-y-2 lg:hidden">
        {WEEKDAYS.map((day, index) => <div key={day} className={cn("rounded-2xl border border-ink/15 bg-white p-4", todayWeekday === index && "border-aqua bg-aqua/10")}>
          ...
          {byDay[index].length ? <div className="mt-3 grid gap-2">{byDay[index].map(({ block, member }, blockIndex) => <Link href={`/people/${member.id}`} key={block.id} className={cn("flex items-center justify-between rounded-xl px-3 py-3 text-sm font-bold", palette[blockIndex % palette.length])}><span>{member.fullName}</span><span className="text-xs font-semibold opacity-65">{formatTime(block.startTime)}–{formatTime(block.endTime)}</span></Link>)}</div> : <p className="mt-3 text-sm text-ink/45">No scheduled blocks</p>}
        </div>)}
      </div>
    </div>
  );
...
export const ScheduledTotals = ({ people, blocks }: { people: Person[]; blocks: BlockWithMember[] }) => {
  ...
  return <div className="mt-7 grid gap-3 sm:grid-cols-[1.1fr_2fr]">
    ...
    <div className="rounded-[24px] border border-ink/15 bg-white p-5"><div className="space-y-3">{totals.map(({ member, hours }, index) => <div key={member.id} className="flex items-center gap-3"><span className={cn("h-2.5 w-2.5 rounded-full", palette[index % palette.length])} /><Link href={`/people/${member.id}`} className="min-w-0 flex-1 truncate text-sm font-bold hover:text-coral">{member.fullName}</Link><span className="text-sm font-semibold text-ink/55">{hours % 1 ? hours.toFixed(1) : hours} h</span></div>)}</div></div>
  </div>;
};
