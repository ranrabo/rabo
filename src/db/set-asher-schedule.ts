...
// Switches Asher's recurring lab schedule to Wed 13:00-15:00 and Fri 13:00-15:00
// starting 2026-09-23 (the Monday slot goes away and Wed/Fri get an hour shorter).
// Past weeks are left untouched: any block that is currently running is capped the
// day before the cutover, and blocks that only start in the future are replaced.
// New blocks reuse Asher's own effective-date ranges (so the winter-break split
// survives), just clamped so nothing starts before the cutover.
//
// Usage:
//   npx tsx src/db/set-asher-schedule.ts [envFile] [--force]
//
...
const CUTOVER = "2026-09-23";
const LAST_OLD_DAY = "2026-09-22";
...
const run = async () => {
  ...
  const candidates = await db
    .select()
    .from(person)
    .where(ilike(person.fullName, "%asher%"))
    .orderBy(asc(person.id));
  if (candidates.length === 0) {
    console.error("\nNo person row matches name ilike '%asher%'.");
    process.exit(1);
  }
  const asher = candidates.find((row) => firstToken(row.fullName) === "asher") ?? candidates[0];
  if (candidates.length > 1) {
    console.log(`\nheads up: ${candidates.length} name matches (${candidates.map((c) => `#${c.id} ${c.fullName}`).join(", ")}); using #${asher.id}`);
  }
  console.log(`\nresolved: #${asher.id} ${asher.fullName}`);

  const existing = await db
    .select()
    .from(weeklyBlock)
    .where(eq(weeklyBlock.personId, asher.id))
  ...
  console.log(`\nDone. Asher now runs Wed 13:00-15:00 and Fri 13:00-15:00 from ${CUTOVER}.`);
};
